...
def parse_data(exp_value, act_value):
    ...
    @return:   case 1: exp_value and actual value is str or int, then return tuple (exp_value,act_value)
    ...
    if isinstance(exp_value, list):
        if not isinstance(act_value, list):
            ...
            for exp in enumerate(exp_value, start=0):
                index = exp[0]
                exp_value[index] = parse_data(exp_value[index], act_value[index])
        ...
        if isinstance(act_value, dict):
            ...
            LOGGER.error("%s,expected: %s , actual: %s", ERROR_CODE['E400005'], exp_value, act_value)
    else:
        LOGGER.error("%s, expected type:%s, actual type %s", ERROR_CODE['E400006'], type(exp_value), type(act_value))
    return exp_value
        ...
        if value[1] is not None or value[1]:
            if value[0] == 'N/A':
                ...
                if value[0] == value[1]:
                    ...
                    flag += 1
                    return "Failure, expect value: " + str(value[0]) + ", return value: " + str(value[1]), flag
...
def get_component_ids_excel(excel_file):
    ...
    id_info_list = []
    for i in range(1, 5):
        # cell_key.append(input_ws.cell(row=1, column=i).value.encode('utf8'))
        cell_key.append(input_ws.cell(row=1, column=i).value)
    row = 2
    while input_ws.cell(row=row, column=1).value:
        cell_value = []
        for i in range(1, 5):
            # cell_value.append(input_ws.cell(row=row, column=i).value.encode('utf-8').replace('\n', ''))
            cell_value.append(input_ws.cell(row=row, column=i).value.encode("utf8").decode("utf8").replace('\n', ''))
...
def create_real_url(url_value, id_dict, config_file):
    ...
    for match in matches:
        ...
        if value['var'] in config_file:
            url_value = url_value.replace('{' + str(value['var']) + '}', str(config_file[value['var']]))

        elif value['var'] in id_dict:
            replaced = 1
            instance_list = id_dict[value['var']]
            for instance in instance_list:
                sgl_url = url_value.replace('{' + str(value['var']) + '}', str(instance))
                LOGGER.debug("replaced url value %s", sgl_url)
                url_list.append(sgl_url)
        else:
            replaced = 2
            LOGGER.error("%s for parameter %s", ERROR_CODE['E300002'], value['var'])
    ...
    for response in response_list:
        ...
        if len(key_list) == 1 and key_list[0] in return_value:
            for i in return_value[key_list[0]]:
                # end_id_list.append(i['@odata.id'].encode('utf-8'))
            ...
            # for index in range(0, len(key_list)):
                if index == len(key_list) - 1:
                    for case in return_value[key_list[index]]:
                        # end_id_list.append(case['@odata.id'].encode('utf-8'))
                    ...
                    if isinstance(return_value, list):
                        ...
                        LOGGER.warning("%s, %s", WARN_CODE['W100002'], type(return_value))
...
def read_row(input_ws, row, config_file):
    ...
    req_body_value = input_ws.cell(row=row, column=config_file["req_body_seq"]).value
    expect_return_code = input_ws.cell(row=row, column=config_file["expect_return_code_seq"]).value
    expect_return_value = input_ws.cell(row=row, column=config_file["expect_return_value_seq"]).value
    ...
    return  pro_value, url_value, req_body_value, expect_return_code, expect_return_value, attr_name
...
def execute_post_url(body, handler, url):
    ...
    if not isinstance(rsp, dict):
        LOGGER.error("%s,%s, expected type %s", ERROR_CODE["E400004"], type(rsp), dict)
        return None
    return rsp
...
def check_component_cnt(expect_return_value, res_list, result):
    ...
    if expect_return_value.__contains__('count'):
        if expect_return_value['count'] == len(res_list):
            ...
            result.update({"count": "Failure, the actual num is " + str(len(res_list))})
    else:
        result.update({"count": "N/A for this case"})
    return result


def parse_test_result(expect_return_value, expect_return_code, actual_result_list, final_result):
    '''
    ...
    @param final_result: returned final result
        parsing the test final_result by comparing expected_value with real test final_result value.
    '''
    return_code_list = []
    return_value_list = []
    flag = 0
    final_result = check_component_cnt(expect_return_value, actual_result_list, final_result)

    for each_result in actual_result_list:
        if each_result is None:
            LOGGER.warning("%s ,skip handling", WARN_CODE['W100003'])
            continue
        temp_result = {}
        LOGGER.debug("current result is %s,result_list is %s", each_result, actual_result_list)
        ...
        for key, value in ex_value.items():
            if key in return_value:
                ...
                LOGGER.error("%s, %s", ERROR_CODE['E500001'], key)
                exp_act_pairs[key] = (value, "Can't find the key {} in return value".format(key))
...
def write_result_2_excel(config_file, input_ws, row, flag, result):
    ...
    '''
    if len(result) == 0:
        input_ws.cell(row=row, column=config_file["detail_result"], value=str('N/A'))
    else:
        input_ws.cell(row=row, column=config_file["detail_result"], value=str(result))
    if flag == 0:
        input_ws.cell(row=row, column=config_file["final_result"], value=str("Success"))
    else:
        input_ws.cell(row=row, column=config_file["final_result"], value=str("Failure"))
    return row

def execute_final_url(config_file, depends_id, http_handler, method, url, req_body):
    '''
...
def run_test_case_yaml(config_file, case_file, depends_id, http_handler):
 ...
 = case['method'], case['URL'], case['request_body'], case['expected_code'], case['expected_result'], case['case_num']

        expected_value = literal_eval(expected_value)

        LOGGER.info("run test case ##%s##", tc_name)
        if tc_name == "7.1.10 get BIOS version":
            LOGGER.debug("debug")
        flag = 0
        final_result = {}
        rsp_list = execute_final_url(config_file, depends_id, http_handler, method, url, req_body)
        if rsp_list is not None and len(rsp_list) > 0:
            return_value_list, return_code_list, final_result, flag = \
            parse_test_result(expected_value, expected_code, rsp_list, final_result)
            ...
            flag += 1
        case['final_result'] = "Success" if flag == 0 else "Failure"
        case['details_result'] = str(final_result) if len(final_result) > 0 else "N/A"
        cases_result.append(case)
        LOGGER.info("writing test final_result for case %s", tc_name)
...
def run_test_case_excel(config_file, case_file, depends_id, http_handler):
    '''
    perform the test case one by one, and write test final_result back to the excel.
    '''
    LOGGER.info("################# start perform test case #####################")
    ...
    row = 2
    while input_ws.cell(row=row, column=1).value:
        method, url, req_body, expected_code, expected_value, tc_name = read_row(input_ws, row, config_file)

        LOGGER.info("run test case ##%s##", tc_name)
        if tc_name == "7.2.6 get harddisk information":
            LOGGER.debug("debug")
        flag = 0
        final_result = {}
        rsp_list = []
        ...
        if rsp_list is not None and len(rsp_list) > 0:
            ...
            final_result.update({'info': return_value_list})
            LOGGER.debug("return_code_list:%s", return_code_list)
            input_ws.cell(row=row, column=config_file["return_code_seq"], value=str(return_code_list))
        else:
            LOGGER.error("%s", ERROR_CODE['E600001'])
            flag += 1

        LOGGER.info("writing test final_result for row %s", row)
        row = write_result_2_excel(config_file, input_ws, row, flag, final_result)
        row += 1
        input_file.save(case_file)
    LOGGER.info("################# end perform test case #####################")


def run(conf_file, case_excel_file=None, depend_yaml_file=None, case_yaml_file=None, file_mode=None):
    '''
    @param conf_file: config.yaml
    @param case_file: excel case file
    ...
    # get bmc info
    bmc_ip, bmc_user, bmc_pwd = config_file["bmc_ip"], config_file["bmc_user"], config_file["bmc_pwd"]
    ...
    if file_mode == "excel":
        run_test_case_excel(config_file, case_excel_file, depends_id, http_handler)
    elif file_mode == "yaml":
        run_test_case_yaml(config_file, case_yaml_file, depends_id, http_handler)
    else:
        LOGGER.error("%s,%s", ERROR_CODE['E200001'], file_mode)
        return None
